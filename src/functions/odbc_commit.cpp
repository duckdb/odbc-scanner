...
static void odbc_commit_function(duckdb_function_info info, duckdb_data_chunk input, duckdb_vector output) noexcept;

namespace odbcscanner {

static void Commit(duckdb_function_info info, duckdb_data_chunk input, duckdb_vector output) {
	(void)info;

	auto conn_arg = Types::ExtractFunctionArg<int64_t>(input, 0);
	if (conn_arg.second) {
		throw ScannerException("'odbc_commit' error: specified ODBC connection must be not NULL");
	}
	int64_t conn_id = conn_arg.first;
	auto conn_ptr = ConnectionsRegistry::Remove(conn_id);

	if (conn_ptr.get() == nullptr) {
		throw ScannerException("'odbc_commit' error: open ODBC connection not found, id: " + std::to_string(conn_id));
	}
...
void OdbcCommitFunction::Register(duckdb_connection conn) {
	auto fun = ScalarFunctionPtr(duckdb_create_scalar_function(), ScalarFunctionDeleter);
	duckdb_scalar_function_set_name(fun.get(), "odbc_commit");
	...
	// callbacks
	duckdb_scalar_function_set_function(fun.get(), odbc_commit_function);
	...
	if (state != DuckDBSuccess) {
		throw ScannerException("'odbc_commit' function registration failed");
	}
}
...
static void odbc_commit_function(duckdb_function_info info, duckdb_data_chunk input, duckdb_vector output) noexcept {
	try {
		odbcscanner::Commit(info, input, output);
