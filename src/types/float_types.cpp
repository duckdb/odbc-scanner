...
template <typename FLOAT_TYPE>
static void FetchAndSetResultInternal(SQLSMALLINT ctype, const std::string &query, HSTMT hstmt, SQLSMALLINT col_idx,
	...
	if (!SQL_SUCCEEDED(ret)) {
		std::string diag = Diagnostics::Read(hstmt, SQL_HANDLE_STMT);
		throw ScannerException("'SQLGetData' for failed, C type: " + std::to_string(ctype) +
...
template <>
void TypeSpecific::FetchAndSetResult<float>(const std::string &query, HSTMT hstmt, SQLSMALLINT col_idx,
                                            duckdb_vector vec, idx_t row_idx) {
	FetchAndSetResultInternal<float>(SQL_C_FLOAT, query, hstmt, col_idx, vec, row_idx);
}

template <>
void TypeSpecific::FetchAndSetResult<double>(const std::string &query, HSTMT hstmt, SQLSMALLINT col_idx,
                                             duckdb_vector vec, idx_t row_idx) {
	FetchAndSetResultInternal<double>(SQL_C_DOUBLE, query, hstmt, col_idx, vec, row_idx);
}
