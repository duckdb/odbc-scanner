...
template <>
void TypeSpecific::BindOdbcParam<std::nullptr_t>(const std::string &query, const std::string &, HSTMT hstmt,
                                                 ScannerParam &param, SQLSMALLINT param_idx) {
	SQLRETURN ret = SQLBindParameter(hstmt, param_idx, SQL_PARAM_INPUT, SQL_C_DEFAULT, param.ExpectedType(), 1, 0,
	...
	if (!SQL_SUCCEEDED(ret)) {
		...
		throw ScannerException(
		    ...
		    " index: " + std::to_string(param_idx) + ", query: '" + query + "', return: " + std::to_string(ret) +
		    ", diagnostics: '" + diag + "'");
	}
}
